#!/usr/bin/env bash
# Register the shellbox tool URL with a running OpenWebUI instance via the
# admin API. Idempotent: re-running keeps the same `info.id=shellbox` entry,
# replacing the previous bearer key.
#
# Use this when:
#   * the OpenWebUI data volume already exists (so TOOL_SERVER_CONNECTIONS env
#     pre-seed is ignored), or
#   * you want to push a fresh SHELLBOX_API_KEY into the live config without
...
[ -n "${SHELLBOX_API_KEY:-}" ] || die "SHELLBOX_API_KEY не задан"
...
PAYLOAD="$(
    CURRENT_JSON="$CURRENT_JSON" SHELLBOX_API_KEY="$SHELLBOX_API_KEY" python3 - <<'PY'
import json, os, sys
...
key = os.environ.get("SHELLBOX_API_KEY", "")
new = {
    "type": "openapi",
...
filtered = [c for c in connections if (c.get("info") or {}).get("id") != "shellbox"]
filtered.append(new)
...
ok "shellbox tool server зарегистрирован/обновлён в OpenWebUI ($BASE)"
warn "Не забудьте включить тул per-user/per-model: chat → ➕ → toggle shellbox."
